...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key,value in loss_dict.items():
            value=value.item()
            self.main_config.writer.add_scalar(key,value,self.global_step)

        if self.use_scheduler:
            lr = self.optimizers().param_groups[0]['lr']
            self.main_config.writer.add_scalar("lr", lr, self.global_step)
    ...
    def full_validation(self, batch, state=None):
        ...
        for key,value in metric_dict.items():
            self.main_config.writer.add_scalar(key,value,self.validation_count)
    ...
    def validation_step(self, batch, batch_idx):
        batch['eeg']=batch['eeg'].to(torch.float32)
