...
class eeg_pretrain_dataset(Dataset):
    def __init__(self, path='../dreamdiffusion/datasets/', roi='VC', patch_size=16, transform=identity, aug_times=2,
    ...
    def __getitem__(self, index):
        ...
        if data.shape[-1] > self.data_len:
            ...
            f = interp1d(x, data)
            data = f(x2)
        # a=ret = np.zeros((data.shape[0], self.data_len))
        # for i in range((self.data_len // data.shape[-1])):
        #     a[:,i * data.shape[-1]: (i + 1) * data.shape[-1]] = data
        # if self.data_len % data.shape[-1] != 0:
        #
        #     a[:, -(self.data_len % data.shape[-1]):] = data[:, : (self.data_len % data.shape[-1])]
        # elif (self.data_len < data.shape[-1]):
        #     idx2 = np.random.randint(0, int(data.shape[-1] - self.data_len) + 1)
        #     a = data[:, idx2: idx2 + self.data_len]
        #     # print(ret.shape)
        # elif (self.data_chan == data.shape[-1]):
        #     a = data
        ...
        if (self.data_chan > data.shape[-2]):
            for i in range((self.data_chan//data.shape[-2])):
                ...
                ret[i * data.shape[-2]: (i+1) * data.shape[-2], :] = a
            if self.data_chan % data.shape[-2] != 0:

                ret[ -(self.data_chan%data.shape[-2]):, :] = a[: (self.data_chan%data.shape[-2]), :]
...
class EEGDataset(Dataset):
    ...
    # Constructor
    def __init__(self, eeg_signals_path, image_transform=identity, subject=999):
        # Load EEG signals
        loaded = torch.load(eeg_signals_path)
        # if opt.subject!=0:
        #     self.data = [loaded['dataset'][i] for i in range(len(loaded['dataset']) ) if loaded['dataset'][i]['subject']==opt.subject]
        ...
        # self.labels = loaded["labels"]
        self.images = loaded["images"]
        # self.imagenet = r'E:\yinda\naodianproject\DreamDiffusion-main\datasets\imageNet_images'
        self.imagenet = r'E:\yinda\naodianproject\DreamDiffusion-main\datasets\yinda\images'
        self.image_transform = image_transform
        self.num_voxels = 440
        self.data_len = 512
        self.data_chan = 128
        # Compute size
        self.size = len(self.data)
        self.processor = AutoProcessor.from_pretrained(r"E:\yinda\tmp")
    ...
    def __getitem__(self, i):
        ...
        if eeg.shape[-1] > self.data_len:
            idx = np.random.randint(0, int(eeg.shape[-1] - self.data_len)+1)

            data = eeg[:, idx: idx+self.data_len]
        else:
        ...
        ret =np.zeros((self.data_chan,self.data_len))
        if (self.data_chan > data.shape[-2]):
            for j in range((self.data_chan//data.shape[-2])):

                ret[j * data.shape[-2]: (j+1) * data.shape[-2], :] = data
            if self.data_chan % data.shape[-2] != 0:
        ...
        # print(ret.shape)
            ...
            ret = data
        image_name = self.images[self.data[i]["image"]]
        image_path = os.path.join(self.imagenet, image_name+'.bmp')
        # print(image_path)
        ...
        return {'eeg': ret, 'image': self.image_transform(image), 'image_raw': image_raw}
        # Return
        # return eeg, label
...
class Splitter:
    ...
    def __init__(self, dataset, split_path, split_num=0, split_name="train", subject=4):
        ...
        # Filter data
        self.split_idx = [i for i in self.split_idx if i <= len(self.dataset.data) and 20 <= self.dataset.data[i]["eeg"].size(0) <= 3000]
        # Compute size
