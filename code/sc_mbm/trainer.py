...
def train_one_epoch(model, data_loader, optimizer, device, epoch, 
    ...
    for data_iter_step, (data_dcit) in enumerate(data_loader):
        ...
        for p, s in zip(pred, samples):
            # print(p[0], s[0])
            a=torch.cat([p[0].unsqueeze(0), s[0].unsqueeze(0)],axis=0)
            b=torch.corrcoef(torch.cat([p[0].unsqueeze(0), s[0].unsqueeze(0)],axis=0))
            c=torch.corrcoef(torch.cat([p[0].unsqueeze(0), s[0].unsqueeze(0)],axis=0))[0,1]
